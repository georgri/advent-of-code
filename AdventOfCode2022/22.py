    ...
    for line in text.split('\n'):
        ...
        if 'L' in line:
            ...
            for c in line:
                if c in 'LR':
                    if numbuf:
                        instructions.append(int(numbuf))
                        numbuf = ''
...
def trymove(m, x, y, xo, yo):
    capx, capy = len(m), len(m[x])

    #print(x, xo, y, yo, capx, capy)

    xorig, yorig = x, y

    x, y = (x + xo) % capx, (y + yo) % capy

    #print(x,y)

    while len(m[x]) <= y or m[x][y] == ' ':
        x, y = (x + xo) % capx, (y + yo) % capy
...
def run(text):
    ...
    # выполняем инструкции одну за другой
    for inst in instructions:
        if isinstance(inst, int):
            # переместиться в направлении curdir
            x, y = godir(m, x, y, curdir, inst)
        else:
