...
def dfs(pattern, i, towels):
    ...
    for t in towels:
        if pattern[i:i + len(t)] == t:
            #print("matched", pattern[i:], t)
            if dfs(pattern, i + len(t), towels):
                return True
        # else:
        #     print(''.join(pattern[i:i + len(t)]), t)

    return False

def solve(text):
    towels, designs = read_input(text)
