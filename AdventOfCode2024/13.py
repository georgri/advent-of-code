...
def solve_puzzle(a,b,prize):
    res = 1_000_000_000

    # res_i, res_j  = -1,-1
    for i in range(101):
        for j in range(101):
            if a[0] * i + b[0] * j == prize[0] and \
                a[1] * i + b[1] * j == prize[1]:
                if res > i * 3 + j:
                    res = i * 3 + j
                    # res_i, res_j = i,j
...
def solve_single(a, prize):
    x,y = a
    if prize[0] % x != 0 or prize[1] %y != 0:
        return -1, False
    if prize[0] // x != prize[1] // y:
        return -1, False
    return prize[0] // x, True
...
def check_enough(a,b,prize, a_count):
    # let a = 5,10
    # let b = 10,5
    # target = 100,100
    # too little a: 
    # a == 1 => target for b = 95, 90.
    # 95 / b[0] = 9.5; 90 - b[1]*9.5 > 0  ==> too little a
    x,y = (prize[0] - a[0] * a_count, prize[1] - a[1] * a_count)

    if b[0] > b[1] or (b[0] == b[1] and a[0] < a[1]):
        t = x/b[0]
        return y - b[1]*t <= 0.0000000001
    
    else :
        t = y/b[1]
        return x - b[0]*t <= 0.0000000001

def solve_puzzle2(a,b,prize):
    mul = 10_000_000_000_000
    prize = (prize[0] + mul, prize[1] + mul)

    # binary search?
    # if cmp(a[0], a[1]) == cmp(b[0], b[1]):
    #     return 0
    ...
    while left <= right:
        ...
        # check_mid: is there enough of a?
        if check_enough(a,b,prize, mid):
            right = mid-1
        else:
            left = mid+1
        
    a_count = right+1
    #print(a_count, check_enough(a,b,prize, a_count), check_enough(a,b,prize, a_count-1))

    target = (prize[0] - a[0]*a_count, prize[1] - a[1] * a_count)
    b_count, ok = solve_single(b, target)
    if ok:
        # verify
        if a_count * a[0] + b_count * b[0] != prize[0] or \
...
def solve2(text):
    ...
    res = 0
    for i, (a,b, prize) in enumerate(inp):
        # print ("case", i)
        # r1, r2 = solve_puzzle(a,b,prize), solve_puzzle2(a,b,prize)
        # if r1 != r2:
        #     print("MISMATCH:", i, a,b, prize, r1, r2)
        r = solve_puzzle2(a,b,prize)
        # if r > 0:
        #     print(i, r)
        res += r
...
# 42470165294817 - too low
# 85202956069456 - too low
# 88584689879723 - right!
