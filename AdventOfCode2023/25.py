...
#     for a,b in edges:
#         if a in graph:
#             to_del = graph[a].index(b)
#             del graph[a][to_del]
#
#         if b in graph:
#             to_del = graph[b].index(a)
#             del graph[b][to_del]
#
#     return graph
#
#
# def find_edges_to_non_reachable(graph, residual_vertices):
#     res = []
#
#     for v in residual_vertices:
#         for neighbour in graph[v]:
#             if neighbour in residual_vertices:
#                 continue
#             res.append((v, neighbour))
#
#     return res


def find_reachable(graph, source, visited_edges):
    # using dfs

    visited_new = set()
    visited_new.add(source)
    ...
    while queue:
        ...
        for neighbour in graph[v]:
            if (v, neighbour) in visited_edges or (neighbour, v) in visited_edges or neighbour in visited_new:
                continue

            visited_new.add(neighbour)
            queue.append(neighbour)

    return visited_new


def find_shortest_path(graph, source, target, visited_edges):

    # use bfs
    visited = dict() # vertex: parent
    queue = [source]
    ...
    while queue:
        ...
        for v in queue:
            for neighbour in graph[v]:
                if neighbour in visited or (v, neighbour) in visited_edges or (neighbour, v) in visited_edges:
                    continue
...
def ford_fulkerson(graph, source, target):
    ...
    # find another shortest path from source to target, write it to visited_edges
    visited_edges = set()

    paths = 0
    for _ in range(4):
        shortest_path = find_shortest_path(graph, source, target, visited_edges)
        ...
        # print(source, target, shortest_path)

    return visited_edges, paths
...
def solve(text):
    ...
    # total 3000 connections.
    # brute forcing is not gonna work

    # something "minimal cut", right?
    # ford fulkerson?
    ...
    for key in graph:
        ...
        visited_edges, maxflow = ford_fulkerson(graph, vertex1, vertex2)
        if maxflow > 3:
            continue

        residual_vertices = find_reachable(graph, vertex1, visited_edges)

        print(len(residual_vertices), len(graph))
        print(len(residual_vertices) * (len(graph)-len(residual_vertices)))
        break
