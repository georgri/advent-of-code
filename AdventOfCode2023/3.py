...
def adjToSymbol(m, x,y):
    for i in (-1,0,1):
        for j in (-1,0,1):
            if 0 <= x+i < len(m) and 0 <= (y+j) < len(m[0]):
                c = m[x+i][y+j]
    ...
    for i, line in enumerate(text):
        ...
        for j, c in enumerate(line):
            if c.isdigit():
                ...
                if number_mode and adjacent:
                    continue

                number_mode = True
                if adjToSymbol(text, i, j):
                    adjacent = True
        ...
        for j, c in enumerate(line):
            if c.isdigit():
                ...
                if len(adjSet) > 0:
                    print(adjSet)
                    for key in adjSet:
                        starMap[key].append(n)
                    adjSet = set()
                n = 0

    print(starMap)
    for key, nums in starMap.items():
        if len(nums) == 2:
            res += nums[0] * nums[1]
