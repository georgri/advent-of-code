...
ratings_map = {'x': 0, 'm': 1, 'a': 2, 's': 3}
reverse_ratings_map = 'xmas'
comp_map = {'<': 0, '>': 1}
    ...
    for line in text.split('\n'):
        ...
        if line[0] == '{':
            ...
            for rating in line.split(','):
                part.append(int(rating.split('=')[1]))
            parts.append(part)
            ...
            for rule in workflow.split(','):
                if ':' in rule:
                    condition, goto = rule.split(':')
                    if '<' in condition:
                        rating, num = condition.split('<')
                        rules.append((goto, ratings_map[rating], comp_map['<'], int(num)))
                    else:
                        rating, num = condition.split('>')
                        rules.append((goto, ratings_map[rating], comp_map['>'], int(num)))
                else:
...
def check_rule(part, rule):
    ...
    if rule[2] == 0:  # less-than
        return part[rule[1]] < rule[3]
...
def eval_intervals(intervals, workflows, workflow_index, rule_index):
    ...
    if len(rule) == 1:
        ...
        if label == 'A':
            ...
            return eval_intervals(intervals, workflows, label, 0)

    else: # len(rule) > 1
        label, interval_ind, operator, num = rule
        a,b = intervals[interval_ind]
        # check condition
        if operator == 0: # [a,b] < num
            if a >= num:
                return eval_intervals(intervals, workflows, workflow_index, rule_index + 1)
            elif b < num:
                return eval_intervals(intervals, workflows, label, 0)
            else:
                int1 = (a, num-1) # True
                ...
                intervals_copy[interval_ind] = int1
                intervals_copy2 = intervals[:]
                intervals_copy2[interval_ind] = int2

                return eval_intervals(intervals_copy, workflows, label, 0) + eval_intervals(intervals_copy2, workflows, workflow_index, rule_index + 1)

        else: # operator == 1:  # [a,b] > num
            if b <= num:
                return eval_intervals(intervals, workflows, workflow_index, rule_index + 1)
            elif a > num:
                return eval_intervals(intervals, workflows, label, 0)
                ...
                return eval_intervals(intervals_copy, workflows, label, 0) + eval_intervals(intervals_copy2,
                                                                                                workflows,
                                                                                                workflow_index,
                                                                                                rule_index + 1)
...
def solve2(text):
    workflows, _ = read(text)

    intervals = [(1,4000), (1,4000), (1,4000), (1,4000)]
    res = eval_intervals(intervals, workflows, 'in', 0)
