...
def adj(m, x, y):
    ...
    allowed_connections = {(0, 1): (('S', 'L', '-', 'F'), ('7', 'J', '-')),  # to the right
                           (0, -1): (('S', '-', '7', 'J'), ('-', 'L', 'F')),  # to the left
                           (1, 0): (('S', '|', 'F', '7'), ('|', 'L', 'J')),  # down
                           (-1, 0): (('S', '|', 'L', 'J'), ('|', 'F', '7')),  # up
                           }

    for xo, yo in allowed_connections:
        allowed_from, allowed_to = allowed_connections[(xo, yo)]
        if c not in allowed_from:
            continue
        if 0 <= x+xo < len(m) and 0 <= y+yo < len(m[0]):
            # check if connection is allowed
            if m[x+xo][y+yo] not in allowed_to:
                continue
...
def solve(text):
    ...
    # do bfs from start
    visited = dict()  # coord to number of steps
    visited[(x, y)] = 0
    step = [(x, y)]
    ...
    while len(step) > 0:
        new_step = []
        for x, y in step:
            num_steps = visited[(x, y)]
            for a, b in adj(m, x, y):
                if (a, b) in visited:
                    continue
                visited[(a, b)] = num_steps + 1
                new_step.append((a, b))

        step = new_step

    print(max(i for _, i in visited.items()))


#solve(text)


def map_x3(m, visited):

    res = [['.']*len(m[0])*3 for _ in range(len(m)*3)]
    ...
    for x, y in visited:
        ...
        for i in range(3):
            for j in range(3):
                #print(xfrom+i, yfrom+j, i, j)
                res[xfrom+i][yfrom+j] = draw[i][j]
...
def solve2(text):
    ...
    # turn visited items to mega map 3*3
    m = map_x3(m, visited)

    # flood-traverse open spaces from edges
    m = flood(m, 0, 0)

    res = 0
    # get nod visited cells that are center of old cells
    for i, line in enumerate(m):
        for j, c in enumerate(line):
            if i % 3 != 1 or j % 3 != 1:
                continue
